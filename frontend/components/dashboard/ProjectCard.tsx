...
export const ProjectCard = ({ project }: ProjectCardProps) => {
    ...
    return (
        <Link href={`/projects/${project.id}`} className="block">
            <Card
                className={`group relative overflow-hidden border-t-4 transition-all hover:-translate-y-0.5 hover:shadow-lg ${statusBorderColors[status]}`}
            >
                <CardContent className="p-5">
                    <div className="flex items-start justify-between">
                        ...
                        <HealthIndicator status={status} size="sm" variant="solid" label={status} />
                    </div>
