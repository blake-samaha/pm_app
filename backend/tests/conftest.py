...
from models import AuthProvider, Project, User, UserRole
...
@pytest.fixture
def client_financials_user(session) -> User:
    """Create a Client + Financials user (senior client stakeholder)."""
    user = User(
        id=uuid4(),
        email="senior.client@acme.com",
        name="Senior Client",
        role=UserRole.CLIENT_FINANCIALS,  # Requires enum update to work
        auth_provider=AuthProvider.GOOGLE,
        is_pending=False,
    )
    session.add(user)
...
def sample_project(session) -> Project:
    ...
    from models.project import HealthStatus, ProjectType
...
def second_project(session) -> Project:
    """Create a second project for multi-project access control tests."""
    from models.project import HealthStatus, ProjectType
...
@pytest.fixture
def project_with_client_assigned(session, sample_project, client_user):
    """Assign client_user to sample_project for access control tests."""
