...
from dependencies import SessionDep, CurrentUser, SettingsDep
...
from exceptions import ResourceNotFoundError, IntegrationError
...
async def sync_jira_only(
    ...
    """
    logger.info("Jira sync requested", project_id=str(project_id), user=current_user.email)
    ...
    try:
        result = await sync_service.sync_jira_issues(project)
...
async def sync_precursive_only(
    ...
    """
    logger.info("Precursive sync requested", project_id=str(project_id), user=current_user.email)
